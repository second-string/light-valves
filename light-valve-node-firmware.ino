...
void setup() {
    ...
    // - start pwm for output valve pins at either fully clear or fully opaque (TODO :: run a fancy startup or bink to indicate stored node ID)
...
void loop() {
    ...
    //   - if a transition interrupt fires, it's 0. Stop timer and set callback for next transition to start 3/4 period bounded timer
    //   - if timer expires w/ no transition interrupt firing, it's a 1. Set callback for next transition to start 3/4 period bounded timer
    ...
    //     - check flag indicating a 1/2 period transition and push a 0 if set, push a 1 if not (need to special case the very first run of this per-message so don't push any data but we do start timer).
    //     - reset flag
    ...
    //
    // - not sure if we should check 8-bit address once we get those bits or continue to read the next 8-bit data value no matter what to prevent dropped data
    ...
    //   - if it's 1/4 overall baud rate period it's a 1. Need to somehow bail out of second transition counting (or still measure it to ensure it's the same 1/4 period)
    //   - if it's 1/2 overall baud rate period, it's a 0. Immediately start the next measurement period on  the next transiton
    /////////////////////////
